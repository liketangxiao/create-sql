...
public class MysqlProcedureInsertDataService extends BaseProcedureService {
    ...
    public void addProcedure(String path, TableInfoVO tableInfoVO) throws SqlException {
        ...
        for (String insertSql : tableInfoVO.insertData.split("\n")) {
            ...
            for (int i = 0; i < valuesArr.length; i++) {
                ...
                value = value.trim();
                if (pattern.matcher(value).find() && ("CREATE_DATE".equals(codeArr[i]) || "UPDATE_DATE".equals(codeArr[i]))) {
